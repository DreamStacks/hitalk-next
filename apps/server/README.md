...
从仓库根目录运行 pnpm dev，会初始化独立的本地数据库并启动前后端。pnpm --filter @hitalk/server build 仅进行 dry-run，不部署。

必要设置：ADMIN_TOKEN、IP_HASH_SALT、WRITE_LIMITER。仓库 wrangler.jsonc 已声明限流绑定。生产部署前必须创建全新 D1 并替换 production.database_id 占位值。
